...
describe('Phase 4 provider failure control', () => {
  ...
  it('times out a provider that stalls after returning streaming headers', async () => { vi.stubGlobal('fetch', vi.fn((_u, init: RequestInit) => Promise.resolve(new Response(new ReadableStream({ start(controller) { init.signal?.addEventListener('abort', () => controller.error(new DOMException('Aborted', 'AbortError'))); } }), { status: 200 })))); const p = createAIProvider({ provider: 'openrouter', model: 'openrouter/free', apiKey: 'placeholder', timeoutMs: 5 })!; const chunks = []; for await (const chunk of p.stream!(request)) chunks.push(chunk); expect(chunks).toContainEqual({ type: 'error', error: 'TIMEOUT' }); });
  ...
  it('includes safe knowledge actions in streamed local results', async () => { const chunks = []; for await (const chunk of streamKaylaChat({ ...validBody, message: 'What version of ForgerEMS can I download?' }, { kaylaConfig: createKaylaConfig({ KAYLA_ENABLED: 'false' }), consumeRequestAllowance: async () => true })) chunks.push(JSON.parse(chunk)); expect(chunks.some(chunk => Array.isArray(chunk.actions) && chunk.actions.length > 0)).toBe(true); });
