...
describe('FDS browser identity smoke contract', () => {
  it('declares one cache-busted FDS icon family without obsolete alternate metadata', () => {
    expect(layout).toContain('href="/favicon.svg?v=fds-r2"');
    expect(layout).toContain('href="/favicon.ico?v=fds-r2"');
    expect(layout).toContain('href="/favicon-48x48.png?v=fds-r2"');
    expect(layout).toContain('href="/apple-touch-icon.png?v=fds-r2"');
    expect(layout).toContain('href="/site.webmanifest?v=fds-r2"');
    expect(layout).not.toContain('rel="alternate icon"');
  });

  it('uses the canonical forge and orbital FDS symbol rather than a text tile or monogram initials', () => {
    expect(favicon).toContain('Canonical FDS Forge and Orbital Brand Symbol');
    expect(favicon).toContain('fds-forge-facet-top');
    expect(favicon).toContain('fds-orbit-primary');
    expect(favicon).not.toContain('<text');
    expect(favicon).not.toContain('#1b53d6');
    expect(favicon).not.toContain('>FDS<');
    // Ensure no letter-monogram path fragments survived
    expect(favicon).not.toContain('l-4 5h-7v3h9');
  });
  ...
  it('version-busts manifest icons and covers install-shortcut sizes', () => {
    expect(manifest.icons).toEqual(expect.arrayContaining([
      expect.objectContaining({ src: '/icon-192.png?v=fds-r2', sizes: '192x192', type: 'image/png' }),
      expect.objectContaining({ src: '/icon-512.png?v=fds-r2', sizes: '512x512', type: 'image/png' })
    ]));
  });
});
